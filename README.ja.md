...
一度インストールすれば、あとは普段通りコーディングするだけ。エージェントがどれだけ無駄な読み込みを避けたかが見えます。APIキー不要、アカウント不要、クラウドバックエンド不要。使用状況の外部送信は一切ありません(ネットワーク通信を行いません)。
...
- **フック自体もトークンを消費します。** デフォルトの`all`モードでは、`claude-hook`は全プロンプトにパック(通常数千トークン)を追加します。このコストは毎回発生し、節約はパックがファイルの全文読み込みを防いだ場合にのみ生まれます。短い会話的なプロンプトが多い場合は、コーディング系プロンプトだけで発火する`--trigger-mode smart`でインストールしてください。
